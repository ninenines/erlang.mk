...
This chapter will explain how to add dependencies to your
project, how they are fetched and built, and how to bundle
them directly in a single repository.

=== Adding dependencies to your project

List the applications you need in `DEPS`. Cowboy is in the
built-in package index, so the name alone is enough:

[source,make]
DEPS = cowboy

The next time you run `make`, Erlang.mk will fetch and compile
Cowboy. Erlang.mk will also ensure Cowboy is available whenever
you use the shell, run tests and any other operations.

You can pin the version without repeating the repository:

[source,make]
DEPS = cowboy
dep_cowboy_commit = 2.19.0

For a project that is not in the index, set `dep_$(NAME)` to
the fetch method and location. Hex packages look like this:

[source,make]
DEPS = jsx
dep_jsx = hex 3.1.0

Git, and the other methods, are listed in the next section.
...
[source,make]
BUILD_DEPS = erlando
dep_erlando = git https://github.com/proger/erlando master
...
For example, to fetch Cowboy with tag 2.19.0 from Git:
...
=== Built-in package index

The index is only a handful of commonly used projects.
`make search` lists them; `make search q=cowboy` filters
that list.

If you need a different fetch method or repository for an
indexed package, for example a fork, set `dep_$(NAME)` as
usual:

[source,make]
DEPS = cowboy
dep_cowboy = git https://github.com/essen/cowboy 2.19.0
...
* The file `$ERLANG_MK_TMP/dep_built/$dep` is removed.
