...
By default Erlang.mk expects `erl` to be in your `$PATH`.
Installing Erlang is not always easy, particularly if you
need a specific version for a specific project.

You can pin an OTP version with `ERLANG_OTP` and let
Erlang.mk fetch and build it via Kerl. The same mechanism
is used by `make ci`. See the xref:kerl[OTP version management]
chapter.
