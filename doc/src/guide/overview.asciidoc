...
Erlang.mk comes with a small xref:deps[built-in package index]
for a handful of commonly used projects. It is a shortcut over
plain dependencies: nothing is installed globally, packages are
only fetched as project-specific deps.

`make search` lists the index; `make search q=cowboy` filters it.
