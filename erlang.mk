...
APP_ID_GIT  ?= git describe --dirty --abbrev=7 --tags --always --first-parent 2>/dev/null || true
APP_VSN_GIT ?= sed 's/^[^0-9\.]*//' <<< \
...
app:: erlc-include ebin/$(PROJECT).app
	$(eval APP_SRC := src/$(PROJECT).app.src)
	$(eval MODULES := $(shell find ebin -type f -name \*.beam \
		| sed "s/ebin\//'/;s/\.beam/',/" | sed '$$s/.$$//'))
	@if [ -z "$$(grep -E '^[^%]*{modules,' $(APP_SRC))" ]; then \
		echo "Empty modules entry not found in $(APP_SRC). Please consult the erlang.mk README for instructions." >&2; \
		exit 1; \
	fi
	$(appsrc_verbose) sed \
	    -e 's/{modules,[[:space:]]*\[\]}/{modules, \[$(MODULES)\]}/' \
	    -e 's/{id,[[:space:]]*"git"}/{id, "$(shell $(APP_ID_GIT))"}/' \
	    -e 's/{vsn,[[:space:]]*"*git"*}/{vsn, "$(shell $(APP_VSN_GIT))"}/' \
	    $(APP_SRC) > ebin/$(PROJECT).app
